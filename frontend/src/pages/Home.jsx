...
const Home = () => {
  ...
  try {
    ...
    const response = await axios.post(
      ...
      {
        content: message,
        user_id: userResponse.data.username, // Явно передаем user_id
      ...
      <Container maxWidth="lg" sx={{
        ...
        <GlassCard>
          ...
          <Typography variant="h5" sx={{
            ...
            Интеллектуальный чат-бот с технологией ИИ
          </Typography>
        ...
        {!isAuth && (
          <Box sx={{
            ...
            {['🤖 ИИ Ассистент', '💾 База данных', '🚀 Технологии'].map((title, index) => (
              <Box key={index} sx={{
                background: `linear-gradient(145deg, rgba(97,218,251,0.15) 0%, rgba(0,255,136,0.15) 100%)`,
                borderRadius: '24px',
                p: 4,
                textAlign: 'center',
                transition: 'all 0.3s ease',
